...
| Requirement 1.8 verified as converter-input byte equality plus structural equality, not as `.drawio` byte equality | Compare the two `.drawio` files byte for byte, as the criterion reads | Measured: `graphviz2drawio.convert` is not deterministic — the same input produced 63 223 and 63 212 bytes across two runs. So `.drawio` byte equality is not a checkable predicate for any input, with or without Inspector_Mode. The export parity test therefore asserts the two things that *are* decidable and that carry the criterion's intent: the bytes handed to the converter are identical, and the resulting documents are structurally equal. The criterion's wording stands; this row records how it is discharged. |
